...
@Name("Assert")
@Description("Checks if a condition is true, otherwise throws an error or a warning with a custom message.")
...
public class EffAssert extends Effect implements RuntimeErrorProducer {
        ...
        if (SkCheese.isSyntaxEnabled("assert")) {
            registry.register(SyntaxRegistry.EFFECT,
                    builder(EffAssert.class)
                            .addPattern("assert <.+> with (:error|warning) [message] %string% [stop:and stop]")
                            .build()
            );
        }
    }
    ...
    public boolean init(Expression<?>[] expressions, int matchedPattern, Kleenean isDelayed, ParseResult parseResult) {
        ...
        shouldStop = parseResult.hasTag("stop");
